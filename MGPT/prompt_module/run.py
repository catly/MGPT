...
train_config = {
    ...
    "gcn_hidden_dim": 32,  # 实验是32，gcn隐藏维度
...
###################################################################
    ...
    "prompt": "FEATURE-WEIGHTED-SUM",  #  实验使用 FEATURE-WEIGHTED-SUM
    "prompt_output_dim": 2,
    "scalar": 1e3,
...
def evaluate(model, data_type, device, config, epoch, c_embedding, label_num, pretrain_embedding, node_label, count, task, debug=False,logger=None, writer=None):
    epoch_step = 1
    total_reg_loss = 0
    total_step = config["epochs"] * epoch_step
    total_bp_loss = 0
    ...
    return mean_reg_loss, mean_bp_loss, evaluate_results, total_time,acc,macrof,weightf,c_embedding,none_accuracy
...
if __name__ == "__main__":
    ...
    if dataset == 'luo':
        ...
        #flow_task = ['drug_protein','drug_disease','drug_se','protein_disease']
        ...
        ###########################################################################
                ...
                mid = np.load('node_data/zheng/10000_node/1-shot/{}/{}_train.npy'.format(task,task), allow_pickle=True)  # 读取文件
                for item in mid:
                    trainset.append(list(item))

                mid = np.load('node_data/zheng/10000_node/1-shot/{}/{}_val.npy'.format(task,task), allow_pickle=True)  # 读取文件
                for item in mid:
                    valset.append(list(item))

                mid = np.load('node_data/zheng/10000_node/1-shot/{}/{}_test.npy'.format(task,task), allow_pickle=True)  # 读取文件
                for item in mid:
        ...
        ##################################################################################################################
                graph = dgl.load_graphs("../get_nodefeature_module/graph_dti/zheng_graph_dti/n(6,3)_all_link_random/graph_all")[0][num]
                ########### 参数敏感性
        ...
        ###################################################################################
                ...
                for count in range(train_config["few_shot_tasknum"]):
                    ...
                    mean_reg_loss, mean_bp_loss, evaluate_results, _time, acctest, macroFtest, weightedFtest, c_embedding,none_accuracy = \
                        evaluate(model, "test", device, train_config, epoch, best_c_embedding,
                                 nodelabelnum, testemb, testlabel, count,task, debug=True, logger=logger, writer=writer)
            ...
            none_all_acc = np.array(none_all_acc)
            acc = np.array(acc)


            print('none_all_acc',np.mean(none_all_acc), 'acc std: ', np.std(none_all_acc))
