...
class Sampler(data.Sampler):
    ...
    def __init__(self, dataset, group_by, batch_size, shuffle, drop_last):
        ...
        for attr in group_by:
            setattr(self, attr, list())  # 为对象的属性赋值
        self.data_size = len(dataset.data)   # 图的个数
        print(self.data_size)
        for x in dataset.data:
            for attr in group_by:
                value = x[attr]
                if isinstance(value, dgl.DGLGraph):
                    getattr(self, attr).append(value.number_of_nodes())  # getattr函数用于获取对象的属性值 ,添加节点数量
    ...
    def make_array(self):
        self.rand = np.random.rand(self.data_size).astype(np.float32)  # 建立一个图的大小的随机数
        if self.data_size == 0:
            types = [np.float32] * len(self.order)
        else:
            types = [type(getattr(self, attr)[0]) for attr in self.order]  # self.order = ['graph', 'rand']
        ...
        array = np.array(
            ...
            dtype=dtype)
        return array  # 返回（图index，随机数）

    def __iter__(self):
        array = self.make_array()
        indices = np.argsort(array, axis=0, order=self.order)  # 对其按照order排序
        batches = [indices[i:i + self.batch_size] for i in range(0, len(indices), self.batch_size)]
        if self.shuffle:
            np.random.shuffle(batches)
        batch_idx = 0
        while batch_idx < len(batches)-1:
            yield batches[batch_idx]  # 返回一个生成器
            batch_idx += 1
