...
def pre_train(model, data_loader, device, config):
    ...
    for batch_id, batch in enumerate(data_loader):
        ...
        #改之前adj = graph.adjacency_matrix()
        #改之前adj = adj.to(device)
        '''print('---------------------------------------------------')
        print('adj: ',adj.size())
        print('pred: ',adj.size())
        print('---------------------------------------------------')'''
...
def train(model, optimizer, scheduler, data_type, device, config, epoch, label_num, pretrain_embedding, node_label, logger=None, writer=None):
    ...
    c_embedding = center_embedding(embedding,node_label)   # 得到两个label的平均特征，即虚拟子图
    ...
    # 改之前：pred=F.log_softmax(distance,dim=1)

    pred = torch.argmax(distance, dim=1, keepdim=True)



    #reg_loss = reg_crit(pred, graph_label_onehot)
    #对NLL LOSS用这个公式，否则用上面的
    reg_loss = reg_crit(pred.float(), node_label.squeeze().type(torch.float).to(device))
    #------------------------------------------------
    reg_loss.requires_grad_(True)
    # 改之前 _pred = torch.argmax(pred, dim=1, keepdim=True)
    accuracy = correctness_GPU(pred, node_label)
    total_acc+=accuracy
...
def evaluate(model, data_type, device, config, epoch, c_embedding, label_num, pretrain_embedding, node_label, debug=False,logger=None, writer=None):
    ...
    reg_loss = reg_crit(pred.type(torch.float), node_label.squeeze().type(torch.float).to(device))
    ...
    if isinstance(config["bp_loss_slp"], (int, float)):
        ...
        #                     lambda1=float(l1))
        neg_slp=0.2
    bp_loss = bp_crit(pred.type(torch.float), node_label.squeeze().type(torch.float).to(device), neg_slp)

    #graph_label_onehot=l2onehot(graph_label)
    #_pred = torch.argmax(pred, dim=1, keepdim=True)
    accuracy = correctness_GPU(pred, node_label)
    eval_pred=pred.cpu().numpy()
...
#######################################################################################################
    ...
    fewshot_dir_train = "D:\DTI\R-HGNN-master\R-HGNN-master\graph_dti\graph_train"
    fewshot_dir_v = "D:\DTI\R-HGNN-master\R-HGNN-master\graph_dti\graph_dev"
    print("Load Few Shot")
    trainset = load_graphs(fewshot_dir_train)
    valueset = load_graphs(fewshot_dir_v)
    testset = load_graphs(fewshot_dir_v)
    '''trainset = np.load(fewshot_dir_train)
    valset = np.load(fewshot_dir_v)
    testset = np.load(fewshot_dir_v)'''
####################################################################################################

    '''trainset = trainset[0][0].ndata["_ID"]
    valset = testset[0][0].ndata["_ID"]
    testset = testset[0][0].ndata["_ID"]
    ...
    trainset = np.load("D:/DTI/R-HGNN-master/data/ENZYMES/nodefewshot/1_trainshot_1_valshot_10_tasks/1/test_dgl_dataset.npy",
                       allow_pickle=True)
    valset = np.load("D:/DTI/R-HGNN-master/data/ENZYMES/nodefewshot/1_trainshot_1_valshot_10_tasks/1/val_dgl_dataset.npy",
                      allow_pickle=True)
    testset = np.load("D:/DTI/R-HGNN-master/data/ENZYMES/nodefewshot/1_trainshot_1_valshot_10_tasks/1/train_dgl_dataset.npy",
                      allow_pickle=True)
    ...
    graph=dgl.load_graphs("D:\DTI\R-HGNN-master\R-HGNN-master\graph_dti\graph_train")[0][0]
    nodelabel=graph.ndata["label"]
    nodelabel = torch.tensor([i[0] for i in nodelabel][:64])
    ...
    for count in range(train_config["few_shot_tasknum"]):
        ...
        for data_type in data_loaders:
            if data_type == "train":
                graph_train = trainset
                data_set = []
                i = load_graphs(fewshot_dir_train)[0][0]
                dic = {}
                dic["graph"] = i
                dic["id"] = 0
                dic["label"] = 0
        ...
        pre_train_model.load_state_dict(torch.load("D:/DTI/R-HGNN-master/dumps/debug/best.pt"))
        ...
        #_trainset = torch.tensor([2,1,6])
        #_valset = torch.tensor([2,1,6])
        #_testset = torch.tensor([2, 1, 6])
