...
#################################################################################
    ...
    graph_DPPNode += random.sample(link_DPPS["drug_protein"], 1600)
    graph_DPPNode += random.sample(notLink_DPPS["drug_protein"], 1600)
    graph_DPPNode += random.sample(link_DPPS["drug_disease"], 20000)
    graph_DPPNode += random.sample(notLink_DPPS["drug_disease"], 20000)
    graph_DPPNode += random.sample(link_DPPS["drug_se"], 20000)
    graph_DPPNode += random.sample(notLink_DPPS["drug_se"], 20000)
    graph_DPPNode += random.sample(link_DPPS["protein_disease"], 20000)
    graph_DPPNode += random.sample(notLink_DPPS["protein_disease"], 20000)
    ...
    node_labels += [[1] for _ in range(1600)]
    node_labels += [[0] for _ in range(1600)]
    node_labels += [[1] for _ in range(20000)]
    node_labels += [[0] for _ in range(20000)]
    node_labels += [[1] for _ in range(20000)]
    node_labels += [[0] for _ in range(20000)]
    node_labels += [[1] for _ in range(20000)]
    node_labels += [[0] for _ in range(20000)]
    ...
    for i in graph_DPPNode:
        if num % 100 == 0:
            print(num)
        if i[1][0] == ("drug","protein"):
            for j in graph_DPPNode[:3200]:
                if i[1][1][0] == j[1][1][0] or i[1][1][1] == j[1][1][1]:
                    if i[0] != j[0]:
                        lis_link_source.append(i[0])
                        lis_link_sink.append(j[0])
            for j in graph_DPPNode[3200:83200]:
                    if i[1][1][0] == j[1][1][0]:
                            lis_link_source.append(i[0])
                            lis_link_sink.append(j[0])
            for j in graph_DPPNode[83200:]:
                    if i[1][1][1] == j[1][1][0]:
                            lis_link_source.append(i[0])
                            lis_link_sink.append(j[0])
        elif i[1][0] == ("drug", "disease"):
            for j in graph_DPPNode[:3200]:
                if i[1][1][0] == j[1][1][0]:
                        lis_link_source.append(i[0])
                        lis_link_sink.append(j[0])
            for j in graph_DPPNode[3200:43200]:
                if i[1][1][0] == j[1][1][0] or i[1][1][1] == j[1][1][1]:
                    if i[0] != j[0]:
                        lis_link_source.append(i[0])
                        lis_link_sink.append(j[0])
            for j in graph_DPPNode[43200:83200]:
                if i[1][1][0] == j[1][1][0]:
                        lis_link_source.append(i[0])
                        lis_link_sink.append(j[0])
            for j in graph_DPPNode[83200:]:
    ...
    save_path = f"../graph_dti/luo_graph_dti/n(6,3)_all_link_random/init_graph_all"
