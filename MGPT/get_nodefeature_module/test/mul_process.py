...
#################################################################################
    ...
    for i in range(nums_drug):
        for j in range(nums_chemical):
            if adj_drug_chemical[i][j] == 1:
                link_DPPS["drug_chemical"].append((("drug","chemical"),torch.tensor([i,j,0,0,0,0])))
                link_num["drug_chemical"] += 1
            else:
                notLink_DPPS["drug_chemical"].append((("drug","chemical"),torch.tensor([i,j,0,0,0,0])))
    ...
    for i in range(nums_drug):
        for j in range(nums_sideeffects):
            if adj_drug_sideeffects[i][j] == 1:
                link_DPPS["drug_sideeffects"].append((("drug","sideeffects"),torch.tensor([i,0,j,0,0,0])))
                link_num["drug_sideeffects"] += 1
            else:
                notLink_DPPS["drug_sideeffects"].append((("drug","sideeffects"),torch.tensor([i,0,j,0,0,0])))
    ...
    for i in range(nums_drug):
        for j in range(nums_substituent):
            if adj_drug_substituent[i][j] == 1:
                link_DPPS["drug_substituent"].append((("drug","substituent"),torch.tensor([i,0,0,j,0,0])))
                link_num["drug_substituent"] += 1
            else:
                notLink_DPPS["drug_substituent"].append((("drug","substituent"),torch.tensor([i,0,0,j,0,0])))
    ...
    for i in range(nums_drug):
        for j in range(nums_target):
            if adj_drug_target[i][j] == 1:
                link_DPPS["drug_target"].append((("drug","target"),torch.tensor([i,0,0,0,j,0])))
                link_num["drug_target"] += 1
            else:
                notLink_DPPS["drug_target"].append((("drug","target"),torch.tensor([i,0,0,0,j,0])))
                notlink_num["drug_target"] += 1

    for i in range(nums_target):
        for j in range(nums_go):
            if adj_target_go[i][j] == 1:
                link_DPPS["target_go"].append((("target","go"),torch.tensor([0,0,0,0,i,j])))
                link_num["target_go"] += 1
            else:
                notLink_DPPS["target_go"].append((("target","go"),torch.tensor([0,0,0,0,i,j])))
    ...
    for i,j in (graph_DPPNode):
        if j[0][0] == "drug" and j[0][1] == "chemical":
            ...
            em = torch.cat((embedding_drug[j[1][0]], embedding_sideeffects[j[1][2]]), dim=0)
        elif j[0][0] == "drug" and j[0][1] == "substituent":
            em = torch.cat((embedding_drug[j[1][0]], embedding_substituent[j[1][3]]), dim=0)
        elif j[0][0] == "drug" and j[0][1] == "target":
            em = torch.cat((embedding_drug[j[1][0]], embedding_target[j[1][4]]), dim=0)
        elif j[0][0] == "target" and j[0][1] == "go":
            em = torch.cat((embedding_target[j[1][4]], embedding_go[j[1][5]]), dim=0)
